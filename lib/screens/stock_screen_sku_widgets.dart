...
class _SkuCompactRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final stockCheckDay = sku.stockCheckDay ?? 1;
    final stockCheck = switch (sku.stockCheckSchedule) {
      StockCheckSchedule.daily => text.t('Daily'),
      StockCheckSchedule.weekly => '${text.t('Weekly')} · ${text.t(['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'][stockCheckDay - 1])}',
      StockCheckSchedule.monthly => '${text.t('Monthly')} · ${text.t('Day')} $stockCheckDay',
    };
    return Pressable(
      onTap: onTap,
