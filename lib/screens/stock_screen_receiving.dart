...
class _StockReceivingPageState extends State<_StockReceivingPage> {
  ...
  Future<void> editSku(StockSku sku) async {
    ...
    final noteController = TextEditingController(
      text: existing?.item.note == 'No remark provided.'
          ? ''
          : existing?.item.note ?? '',
    );
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextField(
                      ...
                      maxLines: 4,
                      decoration: InputDecoration(
                        labelText: text.t('Note (optional)'),
                      ),
                    ),
                    const SizedBox(height: 16),
                    PrimaryButton(
                      text: text.t(
                        existing == null ? 'Add SKU' : 'Update SKU',
                      ),
                      icon: Icons.check_rounded,
                      onPressed: complete
                          ? () {
                              final invoice = double.tryParse(
        ...
        if (skus.isEmpty)
          WhiteCard(
            child: Padding(
              ...
              child: Center(
                child: Text(text.t('No SKU linked to this supplier.')),
              ),
            ),
          )
        else
